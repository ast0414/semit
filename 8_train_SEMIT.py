...
parser = argparse.ArgumentParser(description='SEMIT for MNIST and KMNIST')

parser.add_argument('--pct', type=int, default=1, metavar='N', help='percentage of the labeled samples for dataset2 (default: 1%)')
...
parser.add_argument('--eta', type=float, default=1.0, metavar='S', help='alpha scale (classification loss weight) (default: 1)')
...
parser.add_argument('--save', default='/localscratch/san37/semit/SEMIT', type=str, metavar='S', help='save path')
...
args.save = args.save + "_{}PCT".format(args.pct)
if args.save == '':
...
if args.pct != 1:
    ...
    kannada_x_train_unlabeled, kannada_x_train_labeled, kannada_y_train_unlabeled, kannada_y_train_labeled = train_test_split(kannada_x_train, kannada_y_train, test_size=args.pct/100., stratify=kannada_y_train)
...
kannada_labeled_dataset = CustomTensorDataset(torch.from_numpy(kannada_x_train_labeled).float(), torch.from_numpy(kannada_y_train_labeled).long(), transform=train_transform)
kannada_unlabeled_dataset = CustomTensorDataset(torch.from_numpy(kannada_x_train_unlabeled).float(), torch.from_numpy(kannada_y_train_unlabeled).long(), transform=train_transform)
...
train1_loader = DataLoader(mnist_train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
train2_labeled_loader = DataLoader(kannada_labeled_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
train2_unlabeled_loader = DataLoader(kannada_unlabeled_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
val_loader = DataLoader(kannada_val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

# from dataset2
NUM_SAMPLES = len(kannada_labeled_dataset) + len(kannada_unlabeled_dataset)
NUM_LABELED = len(kannada_labeled_dataset)
LABELED_STEP = len(train2_unlabeled_loader) // len(train2_labeled_loader) + 1

NUM_BATCH = min(len(train1_loader), len(train2_labeled_loader) + len(train2_unlabeled_loader))
...
for epoch in tqdm(range(args.start_epoch, args.epochs), desc='Epoch'):
    ...
    # Data iterators
    train1_iter = iter(train1_loader)
    train2_labeled_iter = iter(train2_labeled_loader)
    train2_unlabeled_iter = iter(train2_unlabeled_loader)
    ...
    for batch_idx in range(NUM_BATCH):
        ...
        data_1, target_1 = next(train1_iter)
        data_1 = data_1.to(device)
        target_1 = target_1.to(device)
        batch_size_1 = data_1.size(0)

        is_supervised = batch_idx % LABELED_STEP == 0
        # get batch from respective dataloader
        if is_supervised:
            try:
                data_2, target_2 = next(train2_labeled_iter)
            except StopIteration:
                try:
                    data_2, _ = next(train2_unlabeled_iter)
                    target_2 = None
                except StopIteration:
                    break
        else:
        ...
        if is_supervised and target_2 is not None:
            loss_c_real_2 = criterion_class(class_real_2, target_2)
            errD_real_2 += ALPHA * loss_c_real_2
            summary_writer.add_scalar('Discriminator_2/Loss_C_Real', loss_c_real_2.item() / batch_size_2, iteration)
            summary_writer.add_scalar('Discriminator_2/Acc_Real', compute_batch_accuracy(class_real_2, target_2), iteration)
        ...
        if is_supervised and target_2 is not None:
            loss_g2_c = criterion_class(class_2_1, target_2)
            errG_2_1 += args.lam0 * ALPHA * loss_g2_c

            summary_writer.add_scalar('Decoder_2/Loss_C', loss_g2_c.item() / batch_size_2, iteration)
            summary_writer.add_scalar('Decoder_2/Acc_Fake', compute_batch_accuracy(class_2_1, target_2), iteration)
    ...
    with torch.no_grad():
        for data, target in val_loader:
            ...
            pred = class_translated.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
